...
const PhotoGalleryModal: React.FC<PhotoGalleryModalProps> = ({ images, initialIndex, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-between text-white p-4 md:p-10 ">
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 mask-linear">
                    {images.map((src, index) => (
                        <button
                            ...
                            className={`relative shrink-0 w-16 h-12 md:w-20 md:h-14 rounded-md overflow-hidden border-2 transition-all ${currentIndex === index ? "border-red-500 scale-105" : "border-transparent opacity-50 hover:opacity-100"
                                }`}
                        >
                            <Image
                                src={src}
